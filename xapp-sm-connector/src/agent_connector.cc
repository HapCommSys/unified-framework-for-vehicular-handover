...
int open_control_socket_agent(const char* dest_ip, const int dest_port) {
  ...
  std::cout << "Agent IP " << agent_ip << " Control Socket " << control_sckfd << std::endl;
...
int send_socket(char* buf, size_t payload_size, std::string dest_ip) {
  ...
  uint32_t net_size = htonl (static_cast<u_int32_t> (payload_size));
  send (control_sckfd, &net_size, sizeof(net_size), 0); // send size first
  char response[3] = {0};
  // recv (control_sckfd, response, sizeof(response), 0); // wait for ack
  ...
  if(sent_size < 0) { // the send returns a size of -1 in case of errors
      ...
      std::cout << "Message sent, size = " << sent_size << std::endl;
  }
