...
void Xapp::startup(SubscriptionHandler &sub_ref) {
    ...
    } else if (strcmp(GNB_ID, "rsu") == 0) {
        // Get gNB list from file
        std::cout << "Getting gNB list from file" << std::endl;
        std::ifstream id_file("rsu_list.txt", ios_base::in);
        if(!id_file) {
            std::cerr << "Error in opening file, gonna crash!" << std::endl;
            exit(-1);
        }
    ...
    ext_control_thr_rx = std::unique_ptr<std::thread>(new std::thread{&Xapp::handle_external_control_message, this, SOCKET_PORT_EXT});  // 7000
    ...
    // char* xx = "2";
	// send_ric_control_request(xx, gnblist[0]);
    // send_ric_control_ts_request("1", "3");
...
void Xapp::handle_rx_msg_agent(std::string agent_ip) {
    ...
    while (true) {
        ...
        std::cout << "DRL_Agent_IP: " << agent_ip << " Agent_Socket: " << it->first << " " << it->second << std::endl; 
        if (it != agentIp_socket.end()) {
            int control_sckfd = it->second;
            int rcv_size = recv(control_sckfd, buf, max_size, 0);
            std::cout << "rcv_size: " << rcv_size << std::endl;
            ...
            if (rcv_size > 0) {
                ...
                char* part1 = nullptr;
                char* part2 = nullptr;
                split_data(buf, rcv_size, &part1, &part2);
                // std::cout << "Part 1: " << part1 << std::endl;
                // std::cout << "Part 2: " << part2 << std::endl;
                send_ric_control_ts_request(part1, part2);
                delete[] part1;
                delete[] part2;
...
void Xapp::send_ric_control_request(char* payload, std::string gnb_id, long int req_id, long int func_id, const char* control_header) {
...
// Send a TS contrl message
void Xapp::send_ric_control_ts_request(const char* ueImsi, char* targetCellId) {
    long int req = 1001;
    long int func_id = 300;
    std::string gnb_id = get_rnib_gnblist()[0];
    send_ric_control_request(targetCellId, gnb_id, req, func_id, ueImsi);
}
...
void split_data(char* buf, int rcv_size, char** part1, char** part2) {
    *part1 = nullptr;
    *part2 = nullptr;
    
    char* comma_ptr = static_cast<char*>(memchr(buf, ',', rcv_size));
    
    if (comma_ptr) {
        size_t part1_len = comma_ptr - buf;
        
        *part1 = new char[part1_len + 1];  
        memcpy(*part1, buf, part1_len);
        (*part1)[part1_len] = '\0';  
        
        size_t part2_len = rcv_size - (part1_len + 1);
        
        if (part2_len > 0) {
            *part2 = new char[part2_len + 1];
            memcpy(*part2, comma_ptr + 1, part2_len);
            (*part2)[part2_len] = '\0';
        } else {
            *part2 = new char[1];
            (*part2)[0] = '\0';
        ...
        *part1 = new char[rcv_size + 1];
        memcpy(*part1, buf, rcv_size);
        (*part1)[rcv_size] = '\0';
