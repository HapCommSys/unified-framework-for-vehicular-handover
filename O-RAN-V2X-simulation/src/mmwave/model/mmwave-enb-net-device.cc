...
std::string
BuildUeListString(const std::map<uint16_t, Ptr<UeManager>>& ueMap)
{
...
MmWaveEnbNetDevice::BuildRicIndicationMessageCuUp(std::string plmId, bool flag){
    ...
    cuUpReport["ue_list"] = BuildUeListString(ueMap);
    ...
    for (auto ue : ueMap)
    {
        ...
        cuUpReport["ue." + std::to_string(imsi)] = std::to_string(pdcpLatency);
        /////////////////////////////////////////////////////////////////////////////
...
{      
    ...
    cuCpReport["ue_list"] = BuildUeListString(ueMap);
    ...
    for (auto ue : ueMap){
        ...
        std::string cuCpMsg = std::to_string(sinrThisCell);
        /////////////////////////////////////////////////////
        ...
        for (std::map<long double, uint16_t>::iterator it = --sortFlipMap.end(); it != --sortFlipMap.begin() && itIndex < nNeighbours; it--){
            ...
            if (cellId != m_cellId){
                ...
                cuCpMsg  += "," + std::to_string(cellId) + "," + std::to_string(sinr);
                //////////////////////////////////////////////////////////////////////
                itIndex++; 
        ...
        cuCpReport.insert(std::make_pair("ue." + std::to_string(imsi), cuCpMsg));
        for (int i = nNeighbours; i < E2SM_REPORT_MAX_NEIGH; i++)
        {
            neighStr += ",,,";
        }
...
MmWaveEnbNetDevice::BuildRicIndicationMessageDu(std::string plmId, uint16_t nrCellId, bool flag){
    ...
    duReport["ue_list"] = BuildUeListString(ueMap);
    ...
    for (auto ue : ueMap)
    {
        ...
        duReport["ue." + std::to_string(imsi)] = std::to_string(macPduUe) + "," + std::to_string(macRetx);
